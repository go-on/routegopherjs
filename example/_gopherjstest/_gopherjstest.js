...
$packages["github.com/go-on/router/route"] = (function() {
	var $pkg = {}, method = $packages["github.com/go-on/method"], strings = $packages["strings"], js = $packages["github.com/gopherjs/gopherjs/js"], ErrPairParams, ErrMissingParam, pseudoRouter, Route, xhr, x, New, Mount;
	...
	Route = $pkg.Route = $newType(0, "Struct", "route.Route", "Route", "github.com/go-on/router/route", function(Methods_, DefinitionPath_, Router_, Id_) {
		this.$val = this;
	...
	New = $pkg.New = function(path, methods) {
		var _map, _key, rt, _ref, _i, m, _key$1;
		if (methods.$length === 0) {
			$panic(new $String("no methods given"));
		}
		...
		while (_i < _ref.$length) {
			...
			if (!(new method.Method(m)).IsKnown()) {
				$panic(new $String("unknown method " + (new method.Method(m)).String()));
			}
	...
	Route.Ptr.prototype.Get = function(callback, params) {
		...
		r = this;
		if (!r.HasMethod("GET")) {
			$panic(new $String("route " + r.DefinitionPath + " is not defined for method " + (new method.Method("GET")).String()));
		}
	...
	Route.Ptr.prototype.Delete = function(callback, params) {
		...
		if (!r.HasMethod("DELETE")) {
			$panic(new $String("route " + r.DefinitionPath + " is not defined for method " + (new method.Method("DELETE")).String()));
		}
	...
	Route.Ptr.prototype.Post = function(data, callback, params) {
		...
		r = this;
		if (!r.HasMethod("POST")) {
			$panic(new $String("route " + r.DefinitionPath + " is not defined for method " + (new method.Method("POST")).String()));
		}
	...
	Route.Ptr.prototype.Patch = function(data, callback, params) {
		...
		if (!r.HasMethod("PATCH")) {
			$panic(new $String("route " + r.DefinitionPath + " is not defined for method " + (new method.Method("PATCH")).String()));
		}
	...
	Route.Ptr.prototype.Put = function(data, callback, params) {
		...
		r = this;
		if (!r.HasMethod("PUT")) {
			$panic(new $String("route " + r.DefinitionPath + " is not defined for method " + (new method.Method("PUT")).String()));
		}
	...
	Route.Ptr.prototype.Options = function(callback, params) {
		...
		if (!r.HasMethod("OPTIONS")) {
			$panic(new $String("route " + r.DefinitionPath + " is not defined for method " + (new method.Method("OPTIONS")).String()));
		}
	...
	Route.Ptr.prototype.mount = function(mountPoint) {
		...
		if (!($interfaceIsEqual(r.Router, $ifaceNil))) {
			$panic(new $String("route " + r.DefinitionPath + " already mounted on" + r.Router.MountPath()));
		}
	...
	Route.Ptr.prototype.URLMap = function(params) {
		var r, mountedPath, parts, _ref, _i, i, part, _tuple, _entry, param, has, x$1;
		r = this;
		...
		while (_i < _ref.$length) {
			...
			if (part.charCodeAt(0) === $pkg.PARAM_PREFIX) {
				...
				if (!has) {
					return ["", (x$1 = new ErrMissingParam.Ptr(part.substring(1), r.MountedPath()), new x$1.constructor.Struct(x$1))];
				}
	...
	$pkg.$init = function() {
		...
		Route.init([["Methods", "Methods", "", ($mapType(method.Method, ($structType([])))), ""], ["DefinitionPath", "DefinitionPath", "", $String, ""], ["Router", "Router", "", ($interfaceType([["MountPath", "MountPath", "", $funcType([], [$String], false)]])), ""], ["Id", "Id", "", $String, ""]]);
		xhr = $ifaceNil;
...
$packages["/home/benny/Entwicklung/gopath/src/github.com/go-on/router/route/example/_gopherjstest"] = (function() {
	var $pkg = {}, method = $packages["github.com/go-on/method"], route = $packages["github.com/go-on/router/route"], Mount, main;
	Mount = $pkg.Mount = function(path) {
		route.Mount(path, new ($sliceType(($ptrType(route.Route))))([$pkg.A, $pkg.B]));
	};
	main = function() {
		Mount("/app");
		console.log($pkg.A.MustURL(new ($sliceType($String))([])));
	};
	$pkg.$run = function($b) {
	...
	$pkg.$init = function() {
		$pkg.A = route.New("/a", new ($sliceType(method.Method))(["GET", "POST"]));
		$pkg.B = route.New("/b/:x/:y", new ($sliceType(method.Method))(["PATCH", "PUT"]));
	};
	return $pkg;
})();
$go($packages["/home/benny/Entwicklung/gopath/src/github.com/go-on/router/route/example/_gopherjstest"].$run, [], true);

})();
//# sourceMappingURL=_gopherjstest.js.map
